...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  ...
  if (c >= 'A' && c <= 'Z') {
    index = (c - 'A' + 11) * 8; // Letras maiúsculas começam no índice 11 * 8
  }
  // Verificar se é uma letra minúscula
  else if (c >= 'a' && c <= 'z') {
    index = (c - 'a' + 37) * 8; // Letras minúsculas começam no índice 37 * 8
  }
  // Verificar se é um número
  else if (c >= '0' && c <= '9') {
    index = (c - '0' + 1) * 8; // Números começam no índice 1 * 8
  }
